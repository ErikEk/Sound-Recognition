...
def _get_training(rnn_logits,label,sequence_length):
    ...
    with tf.name_scope("train"):
        ...
        input('_get_training0')
        #NEW
        #loss = crwl_loss_layer(rnn_logits,label)
        #OLD
    ...
    with tf.Graph().as_default():
        ...
        with tf.device(FLAGS.train_device):
            ...
            logits = model.rnn_layers( features, sequence_length,
                                       mjsynth.num_classes() )
            #tf.summary.scalar('sequence_length',sequence_length)
        ...
        with sv.managed_session(config=session_config) as sess:
            ...
            while step < FLAGS.max_num_steps:
                ...
                [step_loss,image_] = sess.run([train_op,image])
                print('features: ',image_)
                print('features_s: ',image_.shape)
                input('dd')
